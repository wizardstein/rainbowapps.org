...
4. **No accounts for visitors, no in-site payments.** The only auth is the owner's `/admin` sign-in. Donations happen via an external Revolut Business link only (`DONATION_URL` in `lib/site.ts` — owner + accountant decision, 2026-07-02); the site never processes money itself.
